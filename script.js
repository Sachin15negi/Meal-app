        ...
        data.meals.forEach((meal) => {
          list += `
              <div class = "meal-item d-flex flex-column align-items-center" id = "${meal.idMeal}">
                  ...
                  <button type="submit" id="fav-btn" class="btn btn-sm btn-outline-primary favourite-button"> Add To Favourites </button>
              </div>
          `;
        });
      }
...
// function to add meals to favourites
function addToFavourites(event) {
  ...
  // This is to check if the mealId is already present
  if (favouriteMeals.indexOf(mealId) !== -1) {
    return;
  }
  // This is to add id to array and save it back to local storage
